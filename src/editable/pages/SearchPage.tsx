...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block overflow-hidden rounded-[0.55rem] border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
      ...
      <div className="p-5 sm:p-6">
        {!image ? <span className="bg-[var(--slot4-page-text)] px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span> : null}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#f6f4f1)] text-[var(--editable-page-text,#111111)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[0.55rem] border border-[var(--editable-border)] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] md:grid-cols-[0.85fr_1.15fr] lg:p-9">
            <div>
            ...
            <form action="/search" className="self-end rounded-[0.45rem] border border-[var(--editable-border)] bg-[var(--editable-page-bg,#f4f1ec)] p-4 sm:p-5">
              <input type="hidden" name="master" value="1" />
              <label className="flex items-center gap-3 rounded-[0.35rem] border border-[var(--editable-border)] bg-white px-4 py-3">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-2xl bg-[var(--editable-page-text,#111111)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
